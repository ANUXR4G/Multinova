...
class LightsSticks {
  ...
  init() {
    ...
    const aMetrics: number[] = [];
    let colorArray: THREE.Color[];
    if (Array.isArray(options.colors.sticks)) colorArray = options.colors.sticks.map(c => new THREE.Color(c));
...
class App {
  ...
  constructor(container: HTMLElement, options: HyperspeedOptions) {
    ...
    this.useResizeObserver = typeof ResizeObserver !== 'undefined';
  ...
  dispose() {
    ...
    if (this.useResizeObserver && this.resizeObserver) {
      ...
      this.resizeObserver = null;
    } else if (!this.useResizeObserver) {
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = {}, className = '' }) => {
  ...
  return (
    <div 
      id="lights" 
      className={`relative w-full h-screen overflow-hidden flex justify-center ${className}`} 
      ref={hyperspeed}
      style={{ 
        minHeight: '100vh',
      }}
    >
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
        <h1 className="text-[8rem] sm:text-[10rem] md:text-[12rem] lg:text-[14rem] text-white tracking-tighter leading-none select-none px-4 text-center">
          Our Portfolio
        </h1>
      </div>
    </div>
